...
class PincodeChecker extends HTMLElement {
  constructor() {
    ...
    <div id="pincodeCheckerContainer">
      <input type="text" id="pincodeInput" placeholder="Enter your 6-digit pincode" maxlength="6">
      <button id="checkButton">Check</button>
      <div id="resultMessage"></div>
    </div>
  `;
    this.shadowRoot.querySelector('#checkButton').addEventListener('click', this.checkPincode.bind(this));
    this.shadowRoot.querySelector('#pincodeInput').addEventListener('keydown', this.validateKeyPress);
  }

  validateKeyPress(event) {
    // Allow only digits, backspace, tab, arrow keys, delete, and Ctrl/Command key combinations
    if (
      !/^\d$/.test(event.key) &&
  ...
  async checkPincode() {
    const pincodeInput = this.shadowRoot.querySelector('#pincodeInput');
    const pincode = pincodeInput.value;
    const pincodeRegex = /^[1-9][0-9]{5}$/; // Regex to ensure pincode starts with 1-9 and is exactly 6 digits

    if (!pincodeRegex.test(pincode)) {
      this.showToast('Please enter a valid 6-digit pin code.', 'yellow');
      return;
    }
    ...
    try {
      ...
      this.showToast('Error checking pin code.', 'red');
    }
  }

  handleResponse(data) {
    if (data[0].Status === 'Success') {
      const deliveryStatus = data[0].PostOffice.some((po) => po.DeliveryStatus === 'Delivery');
      if (deliveryStatus) {
        this.showToast('Delivery is available for this pin code.', 'green');
      } else {
        this.showToast('Sorry, delivery is not available for this pin code.', 'yellow');
      }
    } else {
      this.showToast('Failed to retrieve pin code details.', 'red');
    }
  }
  ...
  showToast(message, color) {
    ...
    resultMessage.className = 'show ' + color; // Append color class based on status
    setTimeout(() => {
      resultMessage.className = resultMessage.className.replace('show', '');
    }, 3000);
  }
}
